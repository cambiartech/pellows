# Pellows docs

Reusable product & engineering docs. Keep plans here — not scattered in the repo root.

| Doc | Purpose |
|-----|---------|
| **[BUILDS.md](./BUILDS.md)** | **Master checklist — every build to final product** |
| [SCOPE.md](./SCOPE.md) | Product thesis, stack, Realcorp bridge |
| [SEARCH.md](./SEARCH.md) | Multi-agency search architecture |
| [IMPORT_APIS.md](./IMPORT_APIS.md) | Airbnb / Booking / Realcorp link & import |
| [SEO_AI.md](./SEO_AI.md) | Google SEO + AI crawler readiness |
| [ROADMAP.md](./ROADMAP.md) | Diagrams + Detty workflow |
| **[CREDENTIALS.md](./CREDENTIALS.md)** | How to get Stripe / bank / crypto / WhatsApp / OTA keys |
| **[DEPLOY.md](./DEPLOY.md)** | **Netlify + Neon** — build seeds DB; WhatsApp tokens required for phone replies |
| [WHATSAPP_AGENT.md](./WHATSAPP_AGENT.md) | **Text Pellows on WA → book** — Meta setup + deploy |

Root `README.md` stays as the runbook (install / APIs / try now).
